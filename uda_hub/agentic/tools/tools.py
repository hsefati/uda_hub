...
def check_reservations(user_id: str) -> str:
    ...
    try:
        ...
        return "\n".join(
            [f"Event: {row[0]} | Status: {row[1]} | Booked On: {row[2]}" for row in results]
        )
    except Exception as e:
        return f"Database error: {str(e)}"
...
@tool
def save_ticket_summary(ticket_id: str, category: str, tags: list[str], summary: str):
    """
    Saves the final resolution summary and metadata to the udahub.db.
    Use this once the conversation is finished to ensure an audit trail exists.
    """
    try:
        conn = sqlite3.connect("uda_hub/data/core/udahub.db")
        cursor = conn.cursor()
        
        # We update ticket_metadata with the final summary and classification
        cursor.execute("""
        ...
        # We also log the final summary as an internal message for the history
        cursor.execute("""
